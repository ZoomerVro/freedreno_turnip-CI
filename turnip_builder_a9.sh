...
# Build Mode Handling
# =========================
BUILD_MODE="${1:-both}"   # default = both

if [[ "$BUILD_MODE" != "magisk" && "$BUILD_MODE" != "adrenotools" && "$BUILD_MODE" != "both" ]]; then
    echo "Invalid build mode. Use: magisk | adrenotools | both"
    exit 1
fi
...
run_all(){
    ...
    if [[ "$BUILD_MODE" == "magisk" ]]; then
        port_lib_for_magisk
    elif [[ "$BUILD_MODE" == "adrenotools" ]]; then
...
EOF
    ...
    if ! [ -a "$workdir"/turnip.zip ]; then
        ...
        echo -e "$green-All done, the module saved to;$nocolor" && echo "$workdir"/turnip.zip
    fi
}
...
EOF
    ...
    if ! [ -a "$workdir"/turnip_adrenotools.zip ]; then
        echo -e "$red-Packing turnip_adrenotools.zip failed!$nocolor" && exit 1
    else
        echo -e "$green-All done, the module saved to;$nocolor" && echo "$workdir"/turnip_adrenotools.zip
    fi
}
